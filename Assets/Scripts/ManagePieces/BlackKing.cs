...
using ChessBaseAI.Interfaces;
using ChessBaseAI.SpecialMove;

namespace ChessBaseAI.ManagePieces
{
    /// <summary>
    ...
    public sealed class BlackKing : BlackPiece, IKing
    {
        ...
        public override bool MightMove(Board board, int from, int to)
        {
            return
                ...
                (
                    (Math.Abs(Board.Rank(from) - Board.Rank(to)) <= 1 &&
                     Math.Abs(Board.File(from) - Board.File(to)) <= 1) || // the king can move one square 
        ...
        public bool CanCastleLong(Board board, int from, int to)
        {
            return (
                ...
                !board.IsAttackedByWhite(Board.E8) && !board.IsAttackedByWhite(Board.D8) &&
                !board.IsAttackedByWhite(Board.C8) // check if the squares traversed by king are not attacked
            );
        }
        ...
        public bool CanCastleShort(Board board, int from, int to)
        {
            return (
                ...
                !board.IsAttackedByWhite(Board.E8) && !board.IsAttackedByWhite(Board.F8) &&
                !board.IsAttackedByWhite(Board.G8) // check if the squares traversed by king are not attacked
            );
        }
