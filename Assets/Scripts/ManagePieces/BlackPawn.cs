    ...
    public sealed class BlackPawn : BlackPiece, IPawn
    {
        ...
        public bool IsTwoSquaresMove(Board board, int from, int to)
        {
            return
                ...
                board[from + Board.SideSquareNo] == null &&
                board[from + (Board.SideSquareNo << 1)] == null; // the first and second squares are empty
        }
        ...
        public bool IsEnPassantCaptureMove(Board board, int from, int to)
        {
            return
                ...
                to == board.Status.EnPassantTarget.Value && Board.Rank(from) == 4 &&
                Board.Rank(board.Status.EnPassantTarget.Value) == 5 &&
