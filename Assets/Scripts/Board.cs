using System;
using ChessBaseAI.Abstracts;
using ChessBaseAI.ManagePieces;

namespace ChessBaseAI
{
    /// <summary>
    ...
    public class Board
    {
        ...
        public static Board GetStartingBoard()
        {
            ...
            for (int sqIndex = SideSquareNo * (SideSquareNo - 2);
                 sqIndex < SideSquareNo * (SideSquareNo - 1);
                 sqIndex++)
            {
            ...
            status.WhiteCouldCastleLong = status.WhiteCouldCastleShort =
                status.BlackCouldCastleLong = status.BlackCouldCastleShort = true;
        ...
        public bool IsInStartingPosition()
        {
            ...
            for (int sqIndex = SideSquareNo; sqIndex < SideSquareNo * 2; sqIndex++)
            {
                if (!(squares[sqIndex] is BlackPawn))
                {
                    return false;
                }
            }
            ...
            for (int sqIndex = SideSquareNo * (SideSquareNo - 2);
                 ...
                 sqIndex++)
            {
                if (!(squares[sqIndex] is WhitePawn))
                {
                    return false;
                }
            }
            ...
            if (
                status.BlackTurn ||
                !status.WhiteCouldCastleLong || !status.WhiteCouldCastleShort || !status.BlackCouldCastleLong ||
                !status.BlackCouldCastleShort ||
                status.EnPassantTarget != null ||
                status.Ply != 0 ||
                status.Moves != 1)
            {
        ...
        public bool WhiteKingInCheck()
        {
            ...
            for (int sqIndex = 0; sqIndex < SquareNo; sqIndex++)
            {
                if (squares[sqIndex] is WhiteKing)
                {
                    return IsAttackedByBlack(sqIndex);
                }
            }
        ...
        public bool BlackKingInCheck()
        {
            ...
            for (int sqIndex = 0; sqIndex < SquareNo; sqIndex++)
            {
                if (squares[sqIndex] is BlackKing)
                {
                    return IsAttackedByWhite(sqIndex);
                }
            }
